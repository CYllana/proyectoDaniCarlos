...
public class MainScreen extends AbstractScreen {
	...
	public void render (float delta){
		...
		play.update();
		puntuaciones.update();
		miPelota.update(delta);
		miPelotaClose.update(delta);
		miPelotaClasi.update(delta);
		...
		play.setX(miPelota.getPosicion().x);
		play.setY(miPelota.getPosicion().y);
		...
		exit.setX(miPelotaClose.getPosicion().x);
		exit.setY(miPelotaClose.getPosicion().y);

		//clasi
		miPelotaClasi.draw(batch);
		puntuaciones.setX(miPelotaClasi.getPosicion().x);
		puntuaciones.setY(miPelotaClasi.getPosicion().y);
